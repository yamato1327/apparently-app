...
import { ArrowLeft, User, Save, LogOut, Loader2, Pencil, Trash2, Plus, Check, X, Mail, Sun, Moon, Send, Sparkles, Trophy, RefreshCw, Calendar } from "lucide-react";
...
const ProfileSettings = () => {
  ...
  useEffect(() => {
    ...
    const fetchIntegration = async () => {
      const { data } = await supabase
        ...
        .select("gmail_connected, google_email, gmail_last_scanned_at, calendar_connected, calendar_last_synced_at")
        .eq("user_id", user.id)
        .maybeSingle();
      if (data) {
        ...
        setCalendarLastSynced(data.calendar_last_synced_at);
      }
  ...
  const handleReconnectGoogle = async () => {
    ...
    try {
      ...
      toast.error(err.message || "Failed to reconnect Google");
    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-lg px-4 py-6 sm:px-6">
        ...
        <div className="rounded-xl border bg-card p-6 shadow-card mb-6">
          ...
          <p className="text-[11px] text-muted-foreground mb-4">
            Connect Gmail and Google Calendar to automatically find family events — school notices, excursion forms, appointment confirmations.
          </p>
                ...
                {calendarConnected ? (
                  <div className="rounded-lg border bg-muted/10 p-4">
                    <div className="flex items-center justify-between gap-3 flex-wrap">
                      ...
                      <button
                        onClick={() => setShowCalendarResults(true)}
                        className="inline-flex items-center gap-1.5 rounded-full bg-secondary px-3 py-1.5 text-[11px] font-semibold text-secondary-foreground hover:opacity-90 transition-opacity"
                      >
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-foreground">Connect Google Calendar</span>
                  </button>
                )}
              </div>

              {/* Reconnect Google — lets users pick up new scopes */}
              {(gmailConnected || calendarConnected) && (
                <div className="pt-1 border-t border-border">
                  <button
                    onClick={handleReconnectGoogle}
                    disabled={reconnectingGoogle}
                    className="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-lg text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-colors disabled:opacity-50"
                  >
                    {reconnectingGoogle ? (
                    ...
                    Reconnect Google (to grant missing permissions)
                  </button>
                </div>
              )}
            </div>
      ...
      <GoogleCalendarResults open={showCalendarResults} onClose={() => setShowCalendarResults(false)} />
    </div>
  );
};
